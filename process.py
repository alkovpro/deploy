        ...
        while True:
            if not os.path.isfile(PROCESS):
                ...
                if os.path.isfile(PROCESS):
                    ...
                    p = subprocess.Popen("su --command '%s' %s" % (self.CONFIG['GIT_PULL'], APP_USER), shell=True,
                    ...
                    if ('error:' in errmgs) and ('overwritten' in errmgs) and ('aborting' in errmgs):
                        p = subprocess.Popen("su --command '%s' %s" % (self.CONFIG['GIT_RESET'], APP_USER), shell=True,
                                             stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    ...
                    for root, dirs, files in os.walk(os.path.join(DEPLOY_DIR, 'etc')):
                        file_list += [os.path.join(root, name)[len(DEPLOY_DIR):] for name in files if
                                      name not in ['__init__.py', '__init__.pyc', 'config.py', 'config.pyc']]
